...
print_system_specs () {
	...
	sed 's/Hyperthreading=on/Hyperthreading\\033[0;33m on \\033[0m/g' |\
	sed 's/Hyperthreading=off/Hyperthreading\\033[0;31m off \\033[0m/g' |\
...
#------------------------------------------------------------------------------------------------------------
	...
	# set default output
        if [[ -z $1 ]] ; then ShowSystemSpecs=true  ; fi
...
#------------------------------------------------------------------------------------------------------------
	...
	SystemSpecList=$(get_system_specs)
	if [[ -n $ShowSystemSpecs ]]; then print_system_specs && exit ; fi
