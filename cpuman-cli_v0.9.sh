...
echo
echo
	# processing $1 ($SystemSpecList)
	SAVEIFS=$IFS
	IFS=$(echo -en "\n\b")
	for ProsessorConfigLine in $1; do

		HyperthreadStatus=$(awk -F " HyperthreadStatus=" '{printf $2}' <<< $ProsessorConfigLine  | cut -d " " -f1)
		ProcessorNumber=$(awk -F " CPU=" '{printf $2}' <<< $ProsessorConfigLine  | cut -d " " -f1)
		ProcessorStatus=$(awk -F " OnlineStatus=" '{printf $2}' <<< $ProsessorConfigLine  | cut -d " " -f1)
		CoreNumber=$(awk -F " Core=" '{printf $2}' <<< $ProsessorConfigLine  | cut -d " " -f1)
		GovernorStatus=$(awk -F " GovernorStatus=" '{printf $2}' <<< $ProsessorConfigLine  | cut -d " " -f1)
...
#echo "$ProsessorConfigLine"
#echo 
#echo $ProcessorNumber $ProcessorStatus $CoreNumber $GovernorStatus $HyperthreadStatus
#continue

		# configure processors
		if [[ -n $( grep offline <<< "$ProsessorConfigLine") ]]; then
			printf "processor $ProcessorNumber ${Red}offline${Reset}\n"
			echo "0 > /sys/devices/system/cpu/cpu${ProcessorNumber}*/online 2>/dev/null "
		else 
...
#------------------------------------------------------------------------------------------------------------
	...
	for i in $( grep "^ CPU" <<< $SystemSpecList) ; do
		...
		# write CPU status
		echo $OnlineStatus > /sys/devices/system/cpu/cpu$ProcessingCPUNumber/online
	done 
	IFS=$SAVEIFS
