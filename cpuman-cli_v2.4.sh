...
print_parser () {
	...
	if   [[ $(grep SeperatorLine <<< $@) ]]; then
		...
		printf "| %-15s %-2s %-30s %-1s | \n" "$1" "$2" "$3" "$4"
	elif [[ $PrintParser == "print_CPU_data" ]]; then
		printf "| %-7s %-6s %-8s %-10s %-6s %-8s | \n" "$1" "$2" "$3" "$4" "$5" "$6"
...
print_CPU_specs () {
	...
	print_parser "CPU temp actual" "=>" "$CPUCoreTmpAvg C"
...
print_CPU_data () {
	...
	for i in /sys/devices/system/cpu/cpu[0-99]*; do
		...
		# print cpu details
		print_parser  "$CPUStatus" "CPU $CPUNumber" "Core $CoreNumber" "$GovernorStatus" "$CPUFreqActual"
	done
	print_parser SeperatorLine
}
#------------------------------------------------------------------------------------------------------------
...
#	sed 's/CPU cores       =>/CPU cores       =>\'$LYellow' '$MaxCPUs' '\\$Reset'/g' |\
	...
	sed 's/Hyperthreading  => off/Hyperthreading  =>\'$LYellow' off'\\$Reset'/g' |\

	# print cpu status
	sed 's/online/\'$LRed'offline'\\$Reset'/g' |\
	sed 's/offline/\'$LGreen' online'\\$Reset'/g' |\
...
write_configuration () {
	...
	printf "\n"
	print_colored_output "$(print_CPU_specs && print_CPU_data)"
}
#------------------------------------------------------------------------------------------------------------
temp_check () {
...
#------------------------------------------------------------------------------------------------------------
	...
	# check for info output
	if [[ -n $ShowCPUInfoALL ]]; then
		print_colored_output "$(print_CPU_specs && print_CPU_data)"
		exit
	elif [[ -n $ShowCPUSpecs ]]; then
		print_colored_output "$(print_CPU_specs)"
		exit
	elif [[ -n $ShowCPUData ]]; then
		print_colored_output "$(print_CPU_data)"
		exit
	fi
...
#------------------------------------------------------------------------------------------------------------
	...
	if [[ -n $ShowCPUInfoALL ]] && [[ -z $(echo $ShowCPUInfoALL | tr -d [[:digit:]] | tr -d ".") ]]; then
		LoopDelay=$ShowCPUInfoALL
		clear
		tput civis
		while [[ -z $Quit ]] ; do
			tput cup 0,0
