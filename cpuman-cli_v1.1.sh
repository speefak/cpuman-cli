...
# cpunumber eingbe checkken , wenn zahl höher als keren error  weil funktion dann HT aktiviert
	# auto enable hyperthreading for virtual CPU activation
	if [[ $Hyperthreading == "off" ]] && [[ $CPUNumber -ge "$(($MaxCPUs/2))" ]]; then
...
off () {	
	# enable / disable all
	if   [[ -n $EnableAll ]]; then
		if [[ ! $Hyperthreading  == notsupported ]] ;then EnableHT=true ; fi
		EnableCPU=$(seq 0 1 $(($MaxCPUs-1)))		 
	elif [[ -n $DisableAll ]]; then
		if [[ ! $Hyperthreading  == notsupported ]] ;then DisableHT=true ; fi
		DisableCPU=$(seq 0 1 $(($MaxCPUs-1)))
	fi
		#processing_hyperthreading_options	
}
