...
# testing code :
	...
	#printparser
	elif [[ $PrintParser == "print_CPU_data_info_line_fast" ]]; then
	printf "| %-8s %-7s %-39s | \n" "$1" "$2" "$3"


	# print_CPU_specs
	# print core mhz, fast loop output - no status or governor
	SAVEIFS=$IFS
	IFS=$(echo -en "\n\b")
	PrintParser=print_CPU_data_info_line_fast
	CoreNumber="-1"
	for i in $( cat /proc/cpuinfo | awk -F ': ' '/^processor/ { processor = $2 }/^cpu MHz/ { printf "%s @ %.0f\n", processor, $2 }'); do

		CPUNumber=$(cut -d "@" -f1 <<< $i)
		CPUFreqActual=$(cut -d "@" -f2 <<< $i)
		...
		print_parser  "CPU $CPUNumber" "Core $CoreNumber" "$CPUFreqActual MHz"
	done
	IFS=$SAVEIFS
	


 CPUCoreTmpAvg=$(echo $(($(echo "$CPUCoreTemps" | tr  "\n" "+" | sed 's/+$//')))/$CPUCoreCountPhysical | bc)
